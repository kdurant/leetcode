 ...
 * @return
 */
unsigned hashmap_hash_int(hashmap_map* m, char* keystring)
{
...
/**
 * @brief
 *   先计算key的hash值，然后判断表中这个位置是否已经已经存放数据
 * @param in
 * @param key
 * @return int
 */
int hashmap_hash(map_t in, char* key)
{
    ...
    for(i = 0; i < MAX_CHAIN_LENGTH; i++)
    {
        ...
        // 位置已经有数据  且  这个位置数据的key和本次要存放数据的key一致
...
/**
 * @brief 发生哈希冲突后，后续连续的8个位置依然没有空位，则将哈希表容量翻倍
 * 并将先前哈希表的内容重新存放
 *
 * @param in
 * @return int
...
int hashmap_rehash(map_t in)
{
    ...
    curr    = m->data;  // 先保存当前的哈希表地址
    m->data = temp;     // 扩容后的新哈希表
